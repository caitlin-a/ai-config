...
Intended to be used as a submodule of [`dotfiles`](https://github.com/caitlin-a/dotfiles), where `setup.sh` symlinks everything into place so the same config is available on any machine. Changes are auto-committed and pushed daily via `dotfiles/sync.sh`.
